...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch):
    ...
    for batch_idx, (clip, target, _) in enumerate(tqdm(data_loader, desc=header)):
        ...
        total_clip_acc += acc1 * clip.size(0)
        total_loss += loss * clip.size(0)

        # Step the learning rate scheduler
        lr_scheduler.step()

    total_clip_acc = total_clip_acc.item() / len(data_loader.dataset)
    total_loss = total_loss.item() / len(data_loader.dataset)
