MSRACTION_PATH = '/data/iballester/datasets/MSRAction3D_Output'
EXPERIMENTS_PATH = '/caa/Homes01/iballester/dev-svr/3hpe_pc/experiments/'

SYNTHIA_PATH = '/data/iballester/datasets/Synthia4D'

BAD2_PATH = '/data/iballester/datasets/BAD2'

NTU60_PATH = '/data/iballester/datasets/ntu60'

ITOP_SIDE_PATH = '/data/iballester/datasets/ITOP-CLEAN2/SIDE'
#ITOP_TOP_PATH = '/data/iballester/datasets/ITOP/TOP'

#ITOP_SIDE_PRE_PATH = '/data/iballester/datasets/ITOP_pre/SIDE'